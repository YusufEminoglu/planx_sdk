...
def picture_fuzzy_topsis(
    picture_fuzzy_matrix: np.ndarray,
    weights: np.ndarray,
) -> dict[str, Any]:
    ...
    """
    n_alt, n_crit, _ = picture_fuzzy_matrix.shape
    w_norm = weights / np.sum(weights)

    score_mat = picture_fuzzy_matrix[:, :, 0] - picture_fuzzy_matrix[:, :, 2]
